...
def analyze_sentence(sentence):
    ...
    sentiment_score = snlp.sentiments  # 0.0（ネガティブ）〜1.0（ポジティブ）のスコア
...
def write_to_csv(results, output_file):
    with open(output_file, 'w', newline='', encoding='utf-8') as file:
        ...
        # ヘッダー行の書き込み
        writer.writerow(['文番号', '文字数', '単語数', '平均単語長', '感情スコア'])
        # 各文のデータを書き込み
        for i, result in enumerate(results):
            writer.writerow([i+1, result['character_count'], result['word_count'], result['average_word_length'], result['sentiment_score']])
...
def main():
    ...
    for i, sentence in enumerate(sentences):
        if sentence.strip():  # 空白行や空の文をスキップ
            ...
            print(f"  感情スコア: {result['sentiment_score']:.2f}")
            print()
